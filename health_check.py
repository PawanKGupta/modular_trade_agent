...
class HealthCheck:
    ...
    def check(self, name: str, condition: bool, success_msg: str, fail_msg: str, critical: bool = False):
        """
    ...
    def check_version(self):
        ...
        if not HAS_VERSION:
            self.check("Version Module", False, 
                      "Version module available", 
                      "Version module not found", False)
            return
        
        # Package version
        package_version = get_package_version()
        if package_version:
            self.check("Package Version", True,
                      f"{format_version_info(package_version)}",
                      "No package version found", False)
        
        # Installed version
        installed_version = get_installed_version(self.install_dir)
        if installed_version:
            self.check("Installed Version", True,
                      f"{format_version_info(installed_version)}",
                      "No installed version found", False)
        else:
            self.check("Installed Version", False,
                      "Installation found",
                      "No installation detected", False)
    ...
    def check_installation(self):
        ...
        self.check("Install Directory", install_exists,
                  f"Found at {self.install_dir}",
                  f"Not found at {self.install_dir}", True)
        
        if not install_exists:
            self.print_result("✗", "Install Directory", f"Not found at {self.install_dir}")
            return
        
        # Python runtime
        python_dir = self.install_dir / 'python'
        python_exe = python_dir / 'python.exe'
        self.check("Python Runtime", python_exe.exists(),
                  f"Found at {python_exe}",
                  f"Not found at {python_exe}", True)
        
        # Application files
        app_dir = self.install_dir / 'TradingAgent'
        self.check("Application Files", app_dir.exists(),
                  f"Found at {app_dir}",
                  f"Not found at {app_dir}", True)
        
        # Data directory
        data_dir = self.install_dir / 'data'
        self.check("Data Directory", data_dir.exists(),
                  f"Found at {data_dir}",
                  f"Not found at {data_dir}", False)
        
        # Logs directory
        logs_dir = self.install_dir / 'logs'
        self.check("Logs Directory", logs_dir.exists(),
                  f"Found at {logs_dir}",
                  f"Not found at {logs_dir}", False)
    ...
    def check_configuration(self):
        ...
        self.check("Environment File", env_exists,
                  "kotak_neo.env found",
                  "kotak_neo.env missing - credentials not configured!", True)
        ...
        if env_exists:
            ...
            try:
                ...
                for field in required_fields:
                    has_field = field in env_content and not env_content.split(field)[1].split('\n')[0].strip() == '='
                    self.check(f"Config: {field}", has_field,
                              "Configured",
                              "Missing or empty", True)
            except Exception as e:
                self.check("Config Parsing", False,
                          "Config parsed successfully",
                          f"Failed to parse: {e}", True)
        ...
        for service_name in services:
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.check(service_name, False,
                              "Installed",
                              "Not installed", False)
            except Exception as e:
                self.check(service_name, False,
                          "Accessible",
                          f"Error checking: {e}", False)
    ...
    def check_scripts(self):
        ...
        if not (self.install_dir / 'TradingAgent').exists():
            self.check("Scripts", False,
                      "Script directory found",
                      "TradingAgent directory missing", True)
            self.print_result("✗", "Scripts", "TradingAgent directory missing")
            return
    ...
    def check_launcher_scripts(self):
        ...
        for launcher in launchers:
            ...
            self.check(f"Launcher: {launcher}", launcher_path.exists(),
                      "Found",
                      "Missing", False)
    ...
    def check_data_files(self):
        ...
        if not data_dir.exists():
            self.check("Data Files", False,
                      "Data directory exists",
                      "Data directory not found", False)
            self.print_result("✗", "Data Files", "Data directory not found")
            return
        ...
        if history_file.exists():
            try:
                ...
                with open(history_file) as f:
                    ...
                    self.check("Trade History", True,
                              f"{trades_count} trades, {failed_count} failed orders",
                              "", False)
            except Exception as e:
                self.check("Trade History", False,
                          "Valid JSON",
                          f"Corrupt file: {e}", False)
        else:
            self.check("Trade History", False,
                      "File exists",
                      "No trade history yet (normal for new install)", False)
    ...
    def check_logs(self):
        ...
        if not logs_dir.exists():
            self.check("Logs", False,
                      "Logs directory exists",
                      "Logs directory not found (services haven't run yet)", False)
            self.print_result("✗", "Logs", "Logs directory not found (services haven't run yet)")
            return
        
        log_files = list(logs_dir.glob('*.log'))
        
        if log_files:
            self.check("Log Files", True,
            ...
            if log_files:
                ...
                self.check("Latest Log", True,
                          f"{latest_log.name} (modified {mod_time.strftime('%Y-%m-%d %H:%M')})",
                          "", False)
        else:
            self.check("Log Files", False,
                      "Log files exist",
                      "No logs yet (services haven't run)", False)
    ...
    def check_dependencies(self):
        ...
        if not python_exe.exists():
            self.check("Dependencies", False,
                      "Python runtime available",
                      "Python not found", True)
            self.print_result("✗", "Dependencies", "Python not found")
            return
