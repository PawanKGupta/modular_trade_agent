    ...
    try:
        ...
        if chart_quality_enabled:
            ...
            if not chart_quality_data.get("passed", True):
                logger.info(
                    f"{stock_symbol}: Chart quality failed in backtest - {chart_quality_data.get('reason', 'Poor chart quality')}"
                )
                return {
                    "symbol": stock_symbol,
                    "backtest_score": 0.0,
        ...
        for i, (date, row) in enumerate(data.iterrows()):
            ...
            extreme_oversold_threshold = (
                config.rsi_extreme_oversold if hasattr(config, "rsi_extreme_oversold") else 20
            )
            ...
            if (
                ...
                capital_data = liquidity_capital_service.calculate_execution_capital(
                    avg_volume=avg_volume, stock_price=stock_price
                )
                ...
                if execution_capital < 1000:  # Minimum 1K capital
                    logger.debug(
                        f"{stock_symbol}: Skipping trade at {date} - insufficient capital: {execution_capital:.0f}"
                    )
                    continue
            ...
            if current_position is not None:
                ...
                if hit_target or hit_stop:
                    exit_price = (
                        ...
                        if hit_target
                        else current_position["stop_loss"]
                    )
        ...
        logger.info(
            f"Simple backtest for {stock_symbol}: "
...
def add_backtest_scores_to_results(
    ...
    for i, stock_result in enumerate(stock_results, 1):
        try:
            ...
            confidence_level = (
                "High" if trade_count >= 5 else "Medium" if trade_count >= 2 else "Low"
            )
            ...
            if needs_params:
                if (
                    ...
                    logger.info(
                        f"  {ticker}: Recalculating trading parameters (verdict: {stock_result['final_verdict']}, ML: {ml_verdict})"
                    )
                    ...
                    try:
                        ...
                        if current_price and current_price > 0:
                            ...
                            logger.info(
                                f"  {ticker}: Calculated - Buy Range: {buy_range}, Target: {target}, Stop: {stop}"
                            )
                        else:
                            logger.warning(
                        ...
                        if current_price and current_price > 0:
                            stock_result["buy_range"] = (
                                round(current_price * 0.995, 2),
                            ...
                            logger.error(
                                f"  {ticker}: No valid current_price available, cannot set parameters"
                            )
                            stock_result["buy_range"] = None
                            stock_result["target"] = None
                            stock_result["stop"] = None
            ...
            logger.info(
                f"  {ticker}: Current={current_score:.1f}, Backtest={backtest_score:.1f}, Combined={combined_score:.1f}, Final={stock_result['final_verdict']}"
            )
