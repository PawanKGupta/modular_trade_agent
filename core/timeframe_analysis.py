...
class TimeframeAnalysis:
    ...
    def _get_support_lookback(self, timeframe: str) -> int:
        ...
        return (
            self.support_lookback_weekly if timeframe == "weekly" else self.support_lookback_daily
        )
    ...
    def _get_adaptive_lookback(
        self, available_data_days: int, base_lookback: int, timeframe: str
    ) -> int:
        """
    ...
    def analyze_dip_conditions(self, df, timeframe="daily"):
        ...
        support_lookback = self._get_adaptive_lookback(
            available_data_days, support_lookback, timeframe
        )
        ...
        if len(df) < support_lookback:
            ...
            if timeframe == "weekly" and len(df) >= 10:
                ...
                support_lookback = max(
                    int(support_lookback * 0.5), 10
                )  # At least 10, or 50% of requested
                logger.debug(
                    f"Weekly data limited ({len(df)} rows), using reduced lookback: {support_lookback} for {timeframe} analysis"
                )
            else:
                # Not enough data even with reduced lookback
                logger.debug(
                    f"Insufficient data for {timeframe} analysis: {len(df)} rows < {support_lookback} lookback"
                )
                return None
    ...
    def _analyze_oversold_conditions(self, df, timeframe):
        ...
        try:
            ...
            threshold = (
                self.weekly_oversold_threshold if timeframe == "weekly" else self.oversold_threshold
            )
            ...
            rsi_col = f"rsi{self.config.rsi_period}"
            rsi_series = (
                df[rsi_col].tail(10).dropna()
                if rsi_col in df.columns
                else df.get("rsi10", pd.Series()).tail(10).dropna()
            )
            oversold_duration = 0
            if len(rsi_series) > 0:
            ...
            if len(df) >= 10:
                ...
                if len(recent_rsi) >= 5:
                    ...
                    bullish_divergence = (
                        price_new_low and rsi_higher_low and current_rsi < threshold
                    )
                else:
    ...
    def _analyze_selling_pressure(self, df):
        ...
        try:
            ...
            if (
                consecutive_down >= 3
                and not recent_data.iloc[-1]["close"] < recent_data.iloc[-2]["low"] * 0.98
            ):
    ...
    def _analyze_reversion_setup(self, df, timeframe):
        ...
        try:
            ...
            rsi_col = f"rsi{self.config.rsi_period}"
            current_rsi = (
                current_data[rsi_col]
                if rsi_col in current_data.index
                else current_data.get("rsi10")
            )
            ...
            if len(df) >= 2:
                ...
                lower_wick = (
                    current_data["open"] - current_data["low"]
                    if current_data["close"] > current_data["open"]
                    else current_data["close"] - current_data["low"]
                )
                upper_wick = (
                    current_data["high"] - current_data["close"]
                    if current_data["close"] > current_data["open"]
                    else current_data["high"] - current_data["open"]
                )
                ...
                # Bullish engulfing
                if (
                    prev_data["close"] < prev_data["open"]  # Previous red candle
                    and current_data["close"] > current_data["open"]  # Current green candle
                    and current_data["close"] > prev_data["open"]  # Engulfs previous high
                    and current_data["open"] < prev_data["close"]
            ...
            return {"quality": quality, "score": score, "reasons": setup_reasons}
    ...
    def get_dip_buying_alignment_score(self, daily_analysis, weekly_analysis):
        ...
        try:
            ...
            if is_weekly_uptrend:
                ...
                is_near_support = (
                    support_quality in ["strong", "moderate"] and support_distance <= 5.0
                )
            ...
            if daily_support["quality"] == "strong" and daily_support["distance_pct"] <= 3:
                ...
                daily_support["quality"] in ["strong", "moderate"]
                and daily_support["distance_pct"] <= 5
            ):
