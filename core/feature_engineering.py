...
def is_decline_rate_slowing(df: pd.DataFrame, recent_period: int = 5, previous_period: int = 5) -> bool:
    """
...
def count_support_holds(df: pd.DataFrame, lookback: int = 20, tolerance_pct: float = 2.0) -> int:
    ...
    Example:
        Support at ₹950:
        Day 1: Low ₹945, Close ₹955 ✓ (held)
        Day 5: Low ₹940, Close ₹952 ✓ (held)
        Day 8: Low ₹948, Close ₹960 ✓ (held)
...
def calculate_max_drawdown(entry_price: float, daily_lows: list) -> float:
    ...
    Example:
        Entry: ₹1000
        Daily lows: [₹985, ₹970, ₹990]
        Result: -3.0% (worst was ₹970)
    """
