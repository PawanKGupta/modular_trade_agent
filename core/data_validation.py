...
def validate_volume_data(df: pd.DataFrame, ticker: str) -> Dict[str, any]:
    ...
    try:
        ...
        if data_age > 0:
            validation_result['warnings'].append(f"Data is {data_age} day(s) old (latest: {latest_date})")
            if data_age > 2:
        ...
        if zero_volumes > 0:
            ...
            validation_result['volume_issues'].append(f"Zero/negative volumes: {zero_volumes} occurrences")
        ...
        if recent_volume < median_volume * 0.01:  # Less than 1% of median
            validation_result['volume_issues'].append(f"Extremely low recent volume: {recent_volume:,.0f} vs median {median_volume:,.0f}")
        ...
        if not prices.empty:
            ...
            if len(prices) > 1:
                ...
                if price_change > 0.2:  # 20% single-day change
                    validation_result['warnings'].append(f"Large price movement: {price_change:.1%} - verify data accuracy")
...
def get_data_quality_summary(validation_results: Dict[str, Dict]) -> Dict[str, any]:
    ...
    # Count common issues
    stale_data_count = sum(1 for r in validation_results.values() 
                          if r.get('data_age_days', 0) > 0)
    
    volume_issues_count = sum(1 for r in validation_results.values() 
                             if r.get('volume_issues'))
    ...
    return {
        ...
        'validation_success_rate': round(valid_stocks / total_stocks * 100, 1) if total_stocks > 0 else 0
    }
...
def recommend_data_actions(validation_result: Dict) -> List[str]:
    ...
    if not validation_result.get('is_valid', False):
        recommendations.append("⚠️  Data validation failed - avoid trading this stock")
    ...
    if quality_score < 70:
        recommendations.append(f"📉 Low data quality score ({quality_score}/100) - use alternative data")
    elif quality_score < 85:
        recommendations.append(f"⚡ Moderate data quality ({quality_score}/100) - proceed with caution")
