...
def is_truly_large_candle(metrics, market_context, volume_ratio=None):
    ...
    try:
        ...
        price_decline_pct = (metrics['body_size'] / metrics['open']) * 100 if metrics['open'] > 0 else 0
        ...
        for i, (_, candle) in enumerate(recent_candles.iterrows()):
            ...
            if metrics['is_red']:
                ...
                if large_assessment['is_large']:
                    ...
                    assessments_str = ", ".join(large_assessment['assessments'][:2])  # Top 2 reasons
                    details.append(f"Candle {i+1}: Large red candle ({large_assessment['severity']}) - {assessments_str}")
                    
                elif metrics['body_ratio'] > 0.6:  # Still penalize moderate red bodies
                    base_penalty = 1
                    penalty_score += int(base_penalty * age_weight)
                    details.append(f"Candle {i+1}: Moderate red body ({metrics['body_ratio']:.1%} of range)")
            
            # 2. Minimal Lower Wick (No Buyer Support)
            if metrics['lower_wick_ratio'] < 0.1:
                base_penalty = 2
                penalty_score += int(base_penalty * age_weight)
                details.append(f"Candle {i+1}: Minimal lower wick ({metrics['lower_wick_ratio']:.1%}) - no buyer support")
            
            # 3. Full Body Red Candle (Open=High, Close=Low or near)
            if (metrics['is_red'] and 
                metrics['upper_wick_ratio'] < 0.05 and 
                metrics['lower_wick_ratio'] < 0.05):
                base_penalty = 2
                penalty_score += int(base_penalty * age_weight)
        ...
        if 'volume' in recent_candles.columns:
            try:
                ...
                if len(recent_volumes) >= 2:
                    ...
                    if (candle_metrics[0]['is_red'] and 
                        latest_vol > avg_vol * 1.5):
                        penalty_score += 1
...
if __name__ == "__main__":
    ...
    if result['candle_metrics']:
        ...
        print(f"Body Size: {latest['body_size']:.1f} vs Avg Range: {result['market_context']['avg_daily_range']:.1f}")
