...
def run_tests():
    ...
    # Test categories
    test_categories = {
        "1. Unit Tests for Configurable Parameters": ["TestConfigurableParameters"],
        "2. Integration Tests with Current Data": ["TestIntegrationWithData"],
        "3. Backtest Comparison (Old vs New)": ["TestBacktestComparison"],
        "4. Data Fetching Optimization": ["TestDataFetchingOptimization"],
        "5. Indicator Calculation Consistency": ["TestIndicatorConsistency"],
        "6. Performance Benchmarking": ["TestPerformance"],
    }
    ...
    for category, test_classes in test_categories.items():
        ...
        for test_class in test_classes:
            ...
            try:
                ...
                results[test_class] = {"status": "TIMEOUT", "time": 300}
                print(f"??  {test_class}: Timeout (>5 minutes)")
            except Exception as e:
                results[test_class] = {"status": "ERROR", "error": str(e)}
    ...
    for test_class, result in results.items():
        ...
        if status == "PASSED":
            print(
                f"? {test_class}: {result['passed']} passed, {result['failed']} failed ({result['time']:.2f}s)"
            )
        elif status == "FAILED":
