...
def extract_features_at_date(
    ...
    try:
        ...
        logger.debug(
            f"{ticker}: Entry={entry_date}, Signal={signal_date_str} (using signal day for features)"
        )
        ...
        features = {
            ...
            "price_above_ema200": bool(
                pd.notna(last.get("ema200")) and last["close"] > last["ema200"]
            ),
            # Volume features
            ...
            "volume_ratio": (
                float(last["volume"] / df["volume"].tail(20).mean())
                if len(df) >= 20 and df["volume"].tail(20).mean() > 0
                else 1.0
            ),
            "vol_strong": (
                bool(last["volume"] >= 1.5 * df["volume"].tail(20).mean())
                if len(df) >= 20
                else False
            ),
            # Price action
            ...
            "support_distance_pct": (
                float(((last["close"] - df["low"].tail(20).min()) / last["close"]) * 100)
                if len(df) >= 20
                else None
            ),
            # Target distance (EMA9 is the exit target)
            "ema9_distance_pct": (
                float(((last.get("ema9", 0) - last["close"]) / last["close"]) * 100)
                if pd.notna(last.get("ema9")) and last["close"] > 0
                else None
            ),
            # Patterns (simplified)
        ...
        try:
            ...
            logger.debug(
                f"{ticker}: Added dip features (depth={dip_features['dip_depth_from_20d_high_pct']:.1f}%)"
            )
        ...
        try:
            # Parse signal date for temporal features
            signal_datetime = datetime.strptime(signal_date_str, "%Y-%m-%d")

            features["day_of_week"] = signal_datetime.weekday()  # 0=Monday, 6=Sunday
            ...
            logger.debug(
                f"{ticker}: Added time features (day={features['day_of_week']}, month={features['month']})"
            )
        except Exception as e:
            logger.warning(f"{ticker}: Failed to add time features: {e}, using defaults")
        ...
        try:
            ...
            logger.debug(f"{ticker}: Added feature interactions")
        except Exception as e:
            logger.warning(f"{ticker}: Failed to add feature interactions: {e}, using defaults")
...
def create_labels_from_backtest_results_with_reentry(backtest_results: Dict) -> List[Dict]:
    ...
    if isinstance(full_results_str, str):
        try:
            ...
            logger.warning(
                f"Could not parse full_results for {backtest_results.get('ticker')}: {e}"
            )
    ...
    for position in positions:
        ...
        logger.info(
            f"    Processing position: {initial_date_str} ({total_fills} fills, P&L: {pnl_pct:.2f}%)"
        )
        ...
        for fill_idx, fill in enumerate(fills):
            ...
            try:
                ...
                if not features:
                    logger.warning(
                        f"        Failed to extract features for {ticker} at {fill_date_str}"
                    )
                    continue

            except Exception as e:
                logger.warning(
                    f"        Error extracting features for {ticker} at {fill_date_str}: {e}"
                )
                continue
            ...
            if initial_price > 0:
                features["fill_price_vs_initial_pct"] = (
                    (fill_price - initial_price) / initial_price
                ) * 100
            else:
...
def collect_training_data(
    backtest_file: str, output_file: str = "data/ml_training_data_reentry.csv"
) -> pd.DataFrame:
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect training data from backtest results WITH RE-ENTRY EXTRACTION"
    )
    parser.add_argument("--backtest-file", required=True, help="Path to backtest results CSV")
    parser.add_argument(
        "--output", default="data/ml_training_data_reentry.csv", help="Output file path"
    )
