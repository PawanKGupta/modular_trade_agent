...
def extract_features_at_date(
    ...
    try:
        ...
        features = {
            ...
            'volume_ratio': latest['volume'] / df['volume'].rolling(20).mean().iloc[-1] if len(df) >= 20 else 1.0,
        }
...
def main():
    ...
    logger.info(f"Capital per position: ₹{config.user_capital:,.0f} (from config)")
    logger.info("")
