...
def test_lookahead_bias(ticker="RELIANCE.NS", entry_date="2024-01-15"):
    ...
    print(
        f"  Analysis date: {(datetime.strptime(entry_date, '%Y-%m-%d') - timedelta(days=1)).strftime('%Y-%m-%d')} (previous evening)"
    )
    print()
    ...
    df_wrong = data_service.fetch_single_timeframe(
        ticker=ticker, end_date=entry_date, add_current_day=False
    )
    ...
    if df_wrong is not None and not df_wrong.empty:
        ...
        print(
            f"  Date of last row: {last_wrong.name.strftime('%Y-%m-%d') if hasattr(last_wrong.name, 'strftime') else last_wrong.name}"
        )
        print(f"  Close: Rs {last_wrong['close']:.2f}")
        print(f"  Volume: {last_wrong['volume']:,.0f}")
        print(f"  RSI(10): {last_wrong.get('rsi10', 0):.2f}")
        print(f"  High: Rs {last_wrong['high']:.2f}")
        print(f"  Low: Rs {last_wrong['low']:.2f}")
        print()
        print(f"  ? Problem: You DON'T have this data at 9:15 AM on {entry_date}!")
        print(f"     Market hasn't closed yet - you don't know:")
        print(f"     - Final close price (Rs {last_wrong['close']:.2f})")
        print(f"     - Total volume ({last_wrong['volume']:,.0f})")
        print(f"     - Day's high/low")

    # Method 2: CORRECT - Using previous day's close data
    print("\n" + "-" * 80)
    analysis_date = (datetime.strptime(entry_date, "%Y-%m-%d") - timedelta(days=1)).strftime(
        "%Y-%m-%d"
    )
    ...
    df_correct = data_service.fetch_single_timeframe(
        ticker=ticker, end_date=analysis_date, add_current_day=False
    )
    ...
    if df_correct is not None and not df_correct.empty:
        ...
        print(
            f"  Date of last row: {last_correct.name.strftime('%Y-%m-%d') if hasattr(last_correct.name, 'strftime') else last_correct.name}"
        )
        print(f"  Close: Rs {last_correct['close']:.2f}")
        print(f"  Volume: {last_correct['volume']:,.0f}")
        print(f"  RSI(10): {last_correct.get('rsi10', 0):.2f}")
        print(f"  High: Rs {last_correct['high']:.2f}")
        print(f"  Low: Rs {last_correct['low']:.2f}")
        print()
    ...
    if df_wrong is not None and df_correct is not None:
        ...
        volume_diff_pct = (
            (volume_diff / last_correct["volume"]) * 100 if last_correct["volume"] > 0 else 0
        )

        rsi_diff = last_wrong.get("rsi10", 0) - last_correct.get("rsi10", 0)

        print(f"\n  Close price difference: Rs {close_diff:+.2f} ({close_diff_pct:+.2f}%)")
