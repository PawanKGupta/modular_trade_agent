...
class TradeAgentInstaller:
    ...
    def extract_application_files(self):
        ...
        try:
            ...
            if python_src.exists():
                ...
                print(f"  [OK] Extracted Python runtime")
            ...
            print(f"  [FAIL] Failed to extract files: {e}")
            return False
        ...
        for var_name, display_name, required in fields:
            while True:
                ...
                if value or not required:
                    ...
                    print(f"  [FAIL] {display_name} is required!")
    ...
    def _collect_env_gui(self):
        ...
        tk.Label(root, text="Enter your Kotak Neo credentials:", font=("Arial", 10)).pack()
        ...
        for var_name, label_text in fields:
            ...
            entry = ttk.Entry(
                frame, width=30, show="*" if "PASSWORD" in var_name or "MPIN" in var_name else ""
            )
        ...
        ttk.Button(button_frame, text="Install", command=on_submit, width=15).pack(
            side="left", padx=5
        )
        ttk.Button(button_frame, text="Cancel", command=on_cancel, width=15).pack(
            side="left", padx=5
        )
            ...
            try:
                ...
                subprocess.run(
                    [
                        ...
                        service_name,
                        "AppDirectory",
                        str(self.install_dir / "TradingAgent"),
                    ],
                    check=True,
                    capture_output=True,
                    text=True,
                ...
                subprocess.run(
                    [
                        ...
                        service_name,
                        "AppStderr",
                        str(log_dir / f"{service_name}_error.log"),
                    ],
                    check=True,
                    capture_output=True,
                    text=True,
                ...
                print(f"  [OK] Service '{service_name}' installed")
                self.services.append(service_name)
                installed_count += 1

            except subprocess.CalledProcessError as e:
                print(f"  [FAIL] Failed to install service '{service_name}': {e}")
                failed_count += 1
    ...
    def create_launcher_scripts(self):
        ...
        with open(start_all_bat, "w") as f:
            ...
            f.write("pause\n")
        print(f"  [OK] Created: {start_all_bat}")
        ...
        with open(stop_all_bat, "w") as f:
            ...
            f.write("pause\n")
        print(f"  [OK] Created: {stop_all_bat}")
        ...
        print(f"  [OK] Created individual service control scripts")
    ...
    def create_desktop_shortcut(self):
        ...
        try:
            ...
            shortcut.save()
            print(f"  [OK] Created desktop shortcut")
        except:
    ...
    def run_installation(self):
        ...
        if self.installed_version:
            ...
            if self.installer_version == self.installed_version:
                ...
                print(
                    f"Status: Upgrading from {self.installed_version} to {self.installer_version}"
                )
            else:
                print(
                    f"Status: Downgrading from {self.installed_version} to {self.installer_version}"
                )
        else:
        ...
        if save_version(self.installer_version, self.install_dir):
            print(f"  [OK] Saved version: {self.installer_version}")
