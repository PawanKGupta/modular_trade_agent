...
class PerformanceAnalyzer:
    ...
    def _calculate_trade_statistics(self) -> Dict:
        ...
        trade_frequency = len(trades) / ((entry_times.max() - entry_times.min()).days / 30) if len(trades) > 1 else 0
...
Total Invested: ₹{m.get('total_invested', 0):,.0f}
Total P&L: ₹{m.get('total_pnl', 0):,.0f}
...
Average Win: ₹{m.get('avg_win', 0):,.0f}
Average Loss: ₹{m.get('avg_loss', 0):,.0f}
...
Average Trade Size: ₹{m.get('avg_trade_size', 0):,.0f}
Largest Position: ₹{m.get('largest_trade_size', 0):,.0f}
Smallest Position: ₹{m.get('smallest_trade_size', 0):,.0f}
...
Best Month P&L: ₹{m.get('best_month_pnl', 0):,.0f}
Worst Month P&L: ₹{m.get('worst_month_pnl', 0):,.0f}
"""
    ...
    def _generate_recommendations_section(self) -> str:
        ...
        if m.get('profit_factor', 0) < 1.5:
            recommendations.append("• Strategy profitability is marginal - consider risk management improvements")
            
        if m.get('max_drawdown_pct', 0) < -20:
            recommendations.append("• High drawdown detected - implement stop-loss or position sizing rules")
            
        if m.get('strategy_vs_buy_hold', 0) < 0:
            recommendations.append("• Strategy underperforming buy & hold - review entry/exit timing")
            
        if m.get('max_consecutive_losses', 0) > 5:
            recommendations.append("• High consecutive losses - consider trend filtering or market regime detection")
            
        if not recommendations:
            recommendations.append("• Strategy shows good performance characteristics")
            recommendations.append("• Consider optimizing position sizing for better risk-adjusted returns")
