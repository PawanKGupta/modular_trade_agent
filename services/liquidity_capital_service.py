...
class LiquidityCapitalService:
    ...
    def __init__(self, config: Optional[StrategyConfig] = None):
        ...
        self.user_capital = getattr(
            self.config, "user_capital", float(os.getenv("USER_CAPITAL", "200000.0"))
        )
        self.max_position_volume_ratio = getattr(
            self.config,
            "max_position_volume_ratio",
            float(os.getenv("MAX_POSITION_VOLUME_RATIO", "0.10")),
        )
        self.min_absolute_avg_volume = getattr(
            self.config,
    ...
    def calculate_execution_capital(
        ...
        try:
            ...
            max_cap = self.calculate_max_capital(avg_volume, stock_price, max_position_volume_ratio)
            ...
            # Build reason
            if capital_adjusted:
                reason = f"Capital adjusted: Rs {user_cap/1000:.0f}K -> Rs {execution_capital/1000:.0f}K due to liquidity"
            else:
                reason = f"Full capital used: Rs {execution_capital/1000:.0f}K"
    ...
    def is_capital_safe(
        ...
        result = self.calculate_execution_capital(
            user_capital, avg_volume, stock_price, max_position_volume_ratio
        )
        return result.get("is_safe", False)

    def calculate_position_size(self, execution_capital: float, stock_price: float) -> Dict:
        """
        ...
        try:
            ...
            return {"quantity": 0, "actual_capital": 0.0, "execution_capital": execution_capital}
