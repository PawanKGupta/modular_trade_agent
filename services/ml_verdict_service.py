...
class MLVerdictService(VerdictService):
    ...
    def determine_verdict(
        ...
        if self.model_loaded:
            try:
                logger.info(f"ML verdict service: ML model loaded but using rule-based logic (monitoring mode)")
                logger.debug(f"ML verdict service: Chart quality passed - getting ML prediction for monitoring")
                ...
                if ml_result:
                    ...
                    logger.info(f"ML verdict service: ML predicts '{ml_verdict}' ({ml_confidence:.1%} confidence) - monitoring only")
                    logger.debug(f"ML prediction (for monitoring): {ml_verdict}, confidence: {ml_confidence:.1%}, justification: {ml_justification}")
        ...
        if ml_prediction_info:
            ...
            logger.debug(f"Stored ML prediction info: {ml_prediction_info['ml_verdict']} ({ml_prediction_info['ml_confidence']:.1%})")
        else:
    ...
    def _extract_features(
        ...
        features['is_reentry'] = float(indicators.get('is_reentry', 0)) if indicators else 0.0  # False = 0
        features['fill_number'] = float(indicators.get('fill_number', 1)) if indicators else 1.0  # First fill
