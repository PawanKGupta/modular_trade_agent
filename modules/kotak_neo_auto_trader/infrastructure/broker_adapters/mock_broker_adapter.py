...
class MockBrokerAdapter(IBrokerGateway):
    ...
    def __init__(self):
        ...
        self._available_cash = Money.from_int(1000000)  # Rs 10 lakh
    ...
    def search_orders_by_symbol(self, symbol: str) -> List[Order]:
        return [order for order in self._orders.values() if order.symbol.upper() == symbol.upper()]
    ...
    def execute_order(self, order_id: str, execution_price: float):
        ...
        if order_id in self._orders:
            ...
            order.execute(Money.from_float(execution_price), order.quantity)
