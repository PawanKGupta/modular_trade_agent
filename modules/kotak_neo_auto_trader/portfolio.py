            ...
            if isinstance(holdings, dict) and "data" in holdings and holdings["data"]:
                ...
                for holding in holdings_data:
                    ...
                    ltp = _num(
                        ...
                        or holding.get("ltpPrice")
                        or 0
                    )
                    avg_price = _num(
                        holding.get("avgPrice")
                        or holding.get("averagePrice")
                        or holding.get("buyAvg")
                        or holding.get("buyAvgPrice")
                        or 0
                    )
                    market_value = _num(
                        holding.get("marketValue") or holding.get("market_value") or 0
                    )
                    ...
                    if ltp == 0 and quantity > 0:
                        ...
                        if self.price_manager:
                            try:
                                ...
                                websocket_ltp = get_ltp_from_manager(
                                    self.price_manager, lookup_symbol, ticker
                                )
                                if websocket_ltp and websocket_ltp > 0:
                                    ltp = websocket_ltp
                                    logger.debug(
                                        f"{stock_name}: Fetched LTP from WebSocket: Rs {ltp:.2f}"
                                    )
                        ...
                        if ltp == 0 and market_value > 0:
                            ...
                            logger.debug(
                                f"{stock_name}: Using market_value/quantity as LTP: Rs {ltp:.2f}"
                            )
                        ...
                        if ltp == 0:
                            try:
                                ...
                                if yf_ltp > 0:
                                    ltp = yf_ltp
                                    logger.debug(
                                        f"{stock_name}: Fetched LTP from yfinance: Rs {ltp:.2f}"
                                    )
                            except Exception as e:
                                logger.debug(
                                    f"{stock_name}: Could not fetch LTP from yfinance: {e}"
                                )
                    ...
                    pnl = _num(
                        holding.get("pnl")
                        or holding.get("unrealizedPnl")
                        or holding.get("unrealisedPNL")
                        or 0
                    )
                    ...
                    logger.info(
                        f"{stock_name}: Qty={quantity}, LTP=Rs {ltp:.2f}, Value=Rs {market_value:.2f}, P&L=Rs {pnl:.2f}"
                    )
    ...
    def get_limits(self) -> Optional[Dict]:
        ...
        try:
            ...
            if isinstance(limits, dict) and "error" in limits:
                try:
                    msg = (
                        limits["error"][0]["message"]
                        if isinstance(limits.get("error"), list)
                        else limits.get("error")
                    )
                except Exception:
            ...
            logger.info(f" Margin Available: Rs {margin_available}")
