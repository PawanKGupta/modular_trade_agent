...
class KotakNeoMarketData:
    ...
    @handle_reauth
    def get_quote(
        self, symbol: str, exchange: str = "NSE", instrument_token: str = None
    ) -> Optional[Dict]:
        """
        ...
        try:
            ...
            for method_name in method_names:
                ...
                try:
                    # Try with symbol
                    quote = (
                        method(instrument_tokens=instrument_token)
                        if instrument_token
                        else method(
                            exchange_segment=exchange,
                            instrument_tokens=(
                                [symbol]
                                if isinstance(method_name, str) and "quotes" in method_name
                                else symbol
                            ),
                        )
                    )
        ...
        try:
            ...
            for field in ltp_fields:
                if field in data:
                    ...
                    logger.debug(f"{symbol} LTP: Rs {ltp:.2f}")
                    return ltp

            logger.warning(
                f"LTP field not found in quote data for {symbol}. Available fields: {list(data.keys())}"
            )
            return None
