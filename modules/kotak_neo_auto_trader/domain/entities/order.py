...
class Order:
    ...
    def execute(
        ...
        if executed_quantity > self.quantity:
            raise ValueError(
                f"Executed quantity ({executed_quantity}) exceeds order quantity ({self.quantity})"
            )
    ...
    def cancel(self, cancellation_time: datetime | None = None) -> None:
        """
        Cancel the order
    ...
    def _to_iso(self, val: datetime | None | str | None) -> str | None:
        """Return ISO string for datetime or str; avoid calling .isoformat() on str."""
        if val is None:
            return None
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "placed_at": self._to_iso(self.placed_at),
            "executed_at": self._to_iso(self.executed_at),
            "cancelled_at": self._to_iso(self.cancelled_at),
