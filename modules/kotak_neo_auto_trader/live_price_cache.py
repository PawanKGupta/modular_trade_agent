...
class LivePriceCache:
    ...
    def start(self):
        ...
        self._monitor_thread = threading.Thread(
            target=self._connection_monitor, daemon=True, name="WebSocket-Monitor"
        )
        ...
        if self._subscribed_tokens:
            try:
                self.client.un_subscribe(
                    instrument_tokens=self._subscribed_tokens, isIndex=False, isDepth=False
                )
    ...
    def subscribe(self, symbols: List[str]):
        ...
        for symbol in symbols:
            ...
            token_dict = {"instrument_token": token, "exchange_segment": exchange_segment}
        ...
        if not self._ws_connected.is_set():
            ...
            try:
                ...
                if hasattr(self.client, "start_websocket"):
                    ...
                    logger.debug(
                        "No explicit WebSocket start method found, SDK may handle it automatically"
                    )
            except Exception as e:
        ...
        try:
            self.client.subscribe(
                instrument_tokens=tokens_to_subscribe, isIndex=False, isDepth=False
            )
            logger.info(f"[OK] Subscribed to {len(tokens_to_subscribe)} new instruments")
    ...
    def unsubscribe(self, symbols: List[str]):
        ...
        if tokens_to_unsubscribe:
            try:
                self.client.un_subscribe(
                    instrument_tokens=tokens_to_unsubscribe, isIndex=False, isDepth=False
                )
                logger.info(f"[OK] Unsubscribed from {len(tokens_to_unsubscribe)} instruments")
            except Exception as e:
    ...
    def _process_price_update(self, item: Dict):
        ...
        try:
            ...
            logger.debug(f"Updated {symbol}: Rs {ltp}")
