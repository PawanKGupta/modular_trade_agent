            ...
            if lock.acquire(blocking=True, timeout=5.0):
                try:
                    ...
                    try:
                        if auth.force_relogin():
                            logger.info(
                                f"Re-authentication successful (timeout recovery) for {method_name}"
                            )
                            reauth_event.set()
                            return True
                        else:
                            logger.error(
                                f"Re-authentication failed (timeout recovery) for {method_name}"
                            )
                            return False
                    except Exception as e:
                        logger.error(
                            f"Re-authentication exception (timeout recovery) for {method_name}: {e}"
                        )
                        return False
                finally:
                    lock.release()
                ...
                logger.error(
                    f"Could not acquire lock for re-authentication (timeout) for {method_name}"
                )
                return False
        ...
        try:
            ...
            if isinstance(result, dict) and is_auth_error(result):
                ...
                if _check_reauth_failure_rate(self.auth):
                    logger.error(
                        f"Re-authentication blocked for {func.__name__} due to recent failures"
                    )
                    return None
                ...
                if hasattr(self, "auth"):
                    if _attempt_reauth_thread_safe(self.auth, func.__name__):
                        ...
                        try:
                            ...
                            logger.error(
                                f"Method {func.__name__} failed after re-auth: {retry_error}"
                            )
                            return None
                    else:
                        logger.error(f"Re-authentication failed for {func.__name__}")
...
def call_with_reauth(auth, api_call: Callable, *args, **kwargs) -> Any | None:
    """
...
class AuthGuard:
    ...
    def call(self, api_call: Callable, *args, **kwargs) -> Any | None:
