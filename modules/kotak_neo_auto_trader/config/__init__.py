...
if config_file.exists():
    ...
    if spec and spec.loader:
        ...
        TRADES_HISTORY_PATH = getattr(
            kotak_config, "TRADES_HISTORY_PATH", "data/trades_history.json"
        )
        ...
        RECOMMENDED_CSV_GLOB = getattr(
            kotak_config, "RECOMMENDED_CSV_GLOB", "bulk_analysis_final_*.csv"
        )
        ...
        ALLOW_DUPLICATE_RECOMMENDATIONS_SAME_DAY = getattr(
            kotak_config, "ALLOW_DUPLICATE_RECOMMENDATIONS_SAME_DAY", False
        )
        EXIT_ON_EMA9_OR_RSI50 = getattr(kotak_config, "EXIT_ON_EMA9_OR_RSI50", True)
        MIN_QTY = getattr(kotak_config, "MIN_QTY", 1)
        MAX_ORDER_VALUE = getattr(kotak_config, "MAX_ORDER_VALUE", 500000)
