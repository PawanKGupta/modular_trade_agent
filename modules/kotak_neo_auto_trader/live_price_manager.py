...
class LivePriceManager:
    ...
    def subscribe_to_positions(self, symbols: List[str]):
        ...
        try:
            ...
            logger.info(f"[OK] Subscribed to {len(symbols)} positions")
            ...
            if self.price_cache.wait_for_connection(timeout=10):
                ...
                if self.price_cache.wait_for_data(timeout=10):
                    logger.info("[OK] Receiving live data")
                else:
    ...
    def unsubscribe_from_positions(self, symbols: List[str]):
        ...
        try:
            ...
            logger.info(f"[OK] Unsubscribed from {len(symbols)} positions")
        except Exception as e:
    ...
    def print_stats(self):
        ...
        if stats.get("last_websocket_time"):
            print(
                f"Last WebSocket Fetch: {stats['last_websocket_time'].strftime('%Y-%m-%d %H:%M:%S')}"
            )
        if stats.get("last_yfinance_time"):
            print(
                f"Last yfinance Fallback: {stats['last_yfinance_time'].strftime('%Y-%m-%d %H:%M:%S')}"
            )
    ...
    def stop(self):
        ...
        if self.price_cache:
            try:
                ...
                logger.info("[OK] WebSocket cache stopped")
            except Exception as e:
        ...
        if self.auth:
            try:
                self.auth.logout()
                logger.info("[OK] Logged out from Kotak Neo")
            except Exception as e:
