...
for symbol in test_symbols:
    ...
    try:
        ...
        if hasattr(auth.client, 'quote'):
            ...
            try:
                ...
                if isinstance(result, dict):
                    ltp = (
                        result.get('ltp') or
                        result.get('LTP') or 
                        result.get('lastPrice') or
                        result.get('last_price') or
                        result.get('lastTradedPrice')
                    )
                    if ltp:
                        print(f"  ✅ LTP: ₹{ltp}")
                    else:
            ...
            print(f"  Available methods: {[m for m in dir(auth.client) if 'quote' in m.lower() or 'price' in m.lower()]}")
...
quote_related = [m for m in dir(auth.client) if not m.startswith('_') and ('quote' in m.lower() or 'price' in m.lower() or 'ltp' in m.lower())]
if quote_related:
