...
Test flow:
1. Place BUY order: YESBANK @ ₹23.50, qty=2
...
def test_order_modification():
    ...
    if initial_order:
        qty = initial_order.get('quantity') or initial_order.get('qty') or initial_order.get('tradedQuantity', 'N/A')
        ...
        print(f"   Price: ₹{float(price_val):.2f}")
    else:
        print("⚠️  Order not found in order book (may be too fast)")
    
    # Step 4: Modify order quantity
    print("\n📍 Step 4: Modifying order quantity from 2 to 10...")
    new_qty = 10
    
    modify_response = orders.modify_order(
    ...
    if modified_order:
        ...
        qty_val = modified_order.get('quantity') or modified_order.get('qty') or modified_order.get('filledQty') or 0
        current_qty = int(qty_val) if qty_val else 0
        price_val = modified_order.get('price') or modified_order.get('prc') or modified_order.get('avgPrc', 0)
        status_val = modified_order.get('status') or modified_order.get('orderStatus') or modified_order.get('ordSt', 'N/A')
        ...
        if current_qty == new_qty:
            ...
            print(f"\n⚠️  VERIFICATION WARNING: Expected qty={new_qty}, found qty={current_qty}")
    else:
    ...
    if cancel_response:
        ...
        print("⚠️  Please manually cancel order from Kotak Neo app")
        print(f"   Order ID: {order_id}")
    ...
    if final_order:
        ...
        if status in ['cancelled', 'canceled', 'rejected']:
            ...
            print(f"⚠️  Order still active with status: {status}")
    else:
    ...
    print(f"Quantity Verified:  {'✅ Passed' if verification_passed else '⚠️  Warning'}")
    ...
    if verification_passed and cancel_response:
        ...
        print(f"⚠️  Please manually cancel order {order_id}")
        return True
    else:
        print("\n❌ TEST FAILED")
