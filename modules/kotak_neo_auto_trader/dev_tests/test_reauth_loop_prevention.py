...
def test_reauth_loop_prevention():
    ...
    try:
        ...
        assert mock_client.get_orders_count == expected_calls, \
        ...
        assert mock_client.login_count == expected_login_calls, \
            f"Expected {expected_login_calls} login calls, got {mock_client.login_count}"
        print(f"✅ Login attempts: {mock_client.login_count} (initial + 3 re-auth attempts, then blocked)")
