...
test_cases = [
    ...
    ("Midcap ₹800", 800, 20000, 125, True),               # 0.625% < 10% - passes
    ("Midcap ₹800 low vol", 800, 1000, 125, False),       # 12.5% > 10% - fails
    ("Small cap ₹200", 200, 5000, 500, True),             # 10% < 20% - passes
    ("Small cap ₹200 very low", 200, 1000, 500, False),   # 50% > 20% - fails
    ("Penny stock ₹50", 50, 500, 2000, False),            # 400% > 20% - fails (extreme)
]
...
for name, price, avg_vol, qty, expected in test_cases:
    ...
    print(f"\n{name}")
    print(f"  Price: ₹{price:.2f} | Qty: {qty} | Avg Vol: {avg_vol:,}")
...
print("\nKey Insights:")
print("- CURAA (85% ratio) would be filtered ✓")
print("- Good large caps pass easily ✓")
print("- Good mid/small caps pass ✓")
print("- Only truly illiquid stocks filtered ✓")
