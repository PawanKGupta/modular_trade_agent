...
try:
    ts = TrackingScope()
    print("✓ TrackingScope initialized successfully")
...
try:
    ...
    print(f"✓ Successfully loaded data: {type(data)}")
    print(f"✓ Data structure valid: {'symbols' in data}")
    print(f"✓ Symbols in data: {len(data.get('symbols', []))}")
except Exception as e:
...
try:
    ...
    if all_symbols:
        print(f"✓ Symbols: {', '.join(all_symbols[:5])}")
except Exception as e:
...
if os.path.exists(log_file):
    ...
    if bom_errors:
        print(f"⚠ Found {len(bom_errors)} UTF-8 BOM errors in recent logs")
    else:
        print("✓ No UTF-8 BOM errors in recent logs")
else:
    print("✓ Log file not found (OK for first run)")
