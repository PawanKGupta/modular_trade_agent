    ...
    def execute(self, request: BulkAnalysisRequest) -> BulkAnalysisResponse:
        ...
        for ticker in request.tickers:
            try:
                ...
                if result.is_success():
                    logger.debug(f"[OK] {ticker}: {result.verdict}")
                else:
                    logger.warning(f"[FAIL] {ticker}: {result.status}")
        ...
        if request.enable_backtest:
            buyable_count = sum(
                1
                for r in results
                if r.is_buyable(use_final_verdict=True)
                and r.is_success()
                and r.combined_score >= request.min_combined_score
            )
        else:
        ...
        logger.info(
            f"Bulk analysis complete: {successful}/{total_analyzed} successful, {buyable_count} buyable ({execution_time:.2f}s)"
        )
