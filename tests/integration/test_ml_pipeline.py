...
def test_pipeline_without_ml():
    ...
    pipeline = create_analysis_pipeline(
        enable_fundamentals=False, enable_multi_timeframe=False, enable_ml=False
    )
    ...
    if result.errors:
        logger.warning(f"[WARN]? Pipeline had errors: {result.errors}")
        # If there are errors, verdict might be None - that's acceptable for this test
        if verdict is None:
            logger.warning("[WARN]? Verdict is None due to errors - this is acceptable")
...
def test_pipeline_with_ml():
    ...
    if fetch_step:
        with patch.object(fetch_step.data_service, "fetch_single_timeframe", return_value=mock_df):
            ...
            result = pipeline.execute(
                ticker="RELIANCE.NS", config=vars(config) if hasattr(config, "__dict__") else config
            )
    else:
        # Fallback: patch at class level
        with patch.object(DataService, "fetch_single_timeframe", return_value=mock_df):
    ...
    logger.info(
        f"   ML Verdict: {ml_verdict} (confidence: {ml_confidence:.1%})"
        if ml_verdict
        else "   ML Verdict: Not available"
    )
    ...
    if ml_verdict:
        ...
        if verdict_source == "ml":
            ...
            logger.info("   i? Rule-based verdict was used (ML confidence < threshold)")
    else:
        logger.info("\n[WARN]? ML prediction not available (model may not be loaded)")
...
def test_ml_verdict_comparison():
    ...
    for ticker in tickers:
        ...
        # Execute pipeline
        result = pipeline.execute(
            ticker=ticker, config=vars(config) if hasattr(config, "__dict__") else config
        )
    ...
    if ml_used_count > 0:
        agreements = sum(
            1
            for r in results
            if r["ml_verdict"] == r["rule_verdict"] and r["ml_verdict"] is not None
        )
        disagreements = sum(
            1
            for r in results
