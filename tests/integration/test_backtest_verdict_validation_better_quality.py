...
def test_stocks_with_better_chart_quality():
    ...
    for stock_symbol in test_stocks:
        ...
        try:
            result = run_comprehensive_backtest_validation(
                stock_symbol=stock_symbol, years=years, capital_per_position=capital_per_position
            )
            ...
            print(
                f"Verdict validations passed: {verdict_validations.get('passed', 0)}/{result.get('total_signals', 0)}"
            )
            print(
                f"Trade validations passed: {trade_validations.get('passed', 0)}/{result.get('total_signals', 0)}"
            )
            print(
                f"Errors: {len(verdict_validations.get('errors', [])) + len(trade_validations.get('errors', []))}"
            )
            print(
                f"Warnings: {len(verdict_validations.get('warnings', [])) + len(trade_validations.get('warnings', []))}"
            )
    ...
    for stock_symbol, result in results:
        ...
        total_errors += len(verdict_validations.get("errors", [])) + len(
            trade_validations.get("errors", [])
        )
        total_warnings += len(verdict_validations.get("warnings", [])) + len(
            trade_validations.get("warnings", [])
        )

        print(f"{stock_symbol}:")
        print(f"  Signals: {result.get('total_signals', 0)}")
        print(
            f"  Verdict passed: {verdict_validations.get('passed', 0)}/{result.get('total_signals', 0)}"
        )
        print(
            f"  Trade passed: {trade_validations.get('passed', 0)}/{result.get('total_signals', 0)}"
        )
        print(
            f"  Errors: {len(verdict_validations.get('errors', [])) + len(trade_validations.get('errors', []))}"
        )
    ...
    print(
        f"Verdict validations passed: {total_verdict_passed}/{total_signals} ({100*total_verdict_passed/max(total_signals,1):.1f}%)"
    )
    print(
        f"Trade validations passed: {total_trade_passed}/{total_signals} ({100*total_trade_passed/max(total_signals,1):.1f}%)"
    )
    ...
    if total_signals > 0:
        ...
        if verdict_pass_rate >= 90 and trade_pass_rate >= 90:
            ...
            print("[WARN]? TEST PARTIALLY PASSED: Pass rates are acceptable but could be improved")
        else:
