...
class Test2YearBacktestEMAValidation:
    ...
    def test_backtest_engine_2years_ema_warmup(self, sample_stock, date_range_2years, monkeypatch):
        ...
        def mock_fetch_multi_timeframe_data(
            ticker, days=800, end_date=None, add_current_day=True, config=None
        ):
            ...
            return {"daily": filtered_df.reset_index(), "weekly": None}

        monkeypatch.setattr(
            data_fetcher, "fetch_multi_timeframe_data", mock_fetch_multi_timeframe_data
        )
        ...
        engine = BacktestEngine(
            symbol=sample_stock, start_date=start_date, end_date=end_date, config=config
        )
        ...
        print(f"\n[OK] BacktestEngine 2-year test passed:")
    ...
    def test_integrated_backtest_2years_ema_warmup(
        self, sample_stock, date_range_2years, monkeypatch
    ):
        """
        ...
        def mock_fetch_ohlcv_yf(
            ticker, days=365, interval="1d", end_date=None, add_current_day=True
        ):
            if end_date:
                end_dt = pd.to_datetime(end_date)
        ...
        monkeypatch.setattr(data_fetcher, "fetch_ohlcv_yf", mock_fetch_ohlcv_yf)
        ...
        assert (
            "positions" in results or "executed_trades" in results
        ), "Results should contain position/trade information"

        print(f"\n[OK] Integrated Backtest 2-year test passed:")
    ...
    def test_ema_accuracy_validation(self, monkeypatch):
        ...
        def mock_fetch_multi_timeframe_data(
            ...
            return {"daily": filtered_df, "weekly": None}
        ...
        config = BacktestConfig()
        engine = BacktestEngine(
            symbol="TEST.NS", start_date=backtest_start, end_date=backtest_end, config=config
        )
        ...
        print(f"\n[OK] EMA Accuracy Validation passed:")
        print(f"  - EMA200 at start: {first_ema200:.2f}")
        print(f"  - Close at start: {first_close:.2f}")
