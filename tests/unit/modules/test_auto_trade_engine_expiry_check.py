"""
Tests for AutoTradeEngine expiry check before loading signals.

Verifies that AutoTradeEngine calls mark_time_expired_signals() before
...
@pytest.fixture
def auto_trade_engine(mock_auth, db_session):
...
class TestAutoTradeEngineExpiryCheck:
    """Test that AutoTradeEngine checks expiry before loading signals"""

    def test_load_latest_recommendations_calls_expiry_check(self, auto_trade_engine, db_session):
        ...
        with patch("src.infrastructure.persistence.signals_repository.ist_now") as mock_ist_now:
            ...
            with patch(
                ...
                recommendations = auto_trade_engine.load_latest_recommendations()
    ...
    def test_load_latest_recommendations_excludes_expired_signals(
        self, auto_trade_engine, db_session
    ):
        ...
        # Create an active signal (created today)
        active_signal = Signals(
            symbol="TCS.NS",
            status=SignalStatus.ACTIVE,
            ts=datetime(2025, 12, 2, 14, 0, 0, tzinfo=IST),  # Tuesday 2:00 PM
            verdict="buy",
        )
        ...
        with patch("src.infrastructure.persistence.signals_repository.ist_now") as mock_ist_now:
            ...
            # Verify active signal is in recommendations (if any returned)
            # Note: This depends on the filtering logic in load_latest_recommendations
    ...
    def test_load_latest_recommendations_handles_no_signals(self, auto_trade_engine, db_session):
        """Test that load_latest_recommendations handles case with no signals"""
        # Mock ist_now
        with patch("src.infrastructure.persistence.signals_repository.ist_now") as mock_ist_now:
            mock_ist_now.return_value = datetime(2025, 12, 2, 14, 0, 0, tzinfo=IST)

            # Load recommendations (should not crash)
            recommendations = auto_trade_engine.load_latest_recommendations()
