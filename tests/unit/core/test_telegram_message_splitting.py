...
class TestTelegramMessageSplitting:
    ...
    def test_long_message_split_at_stock_boundaries(self, mock_send):
        ...
        for call_args in mock_send.call_args_list:
            ...
            stock_count = sum(1 for line in lines if line.strip() and 
                            line.strip()[0].isdigit() and '. ' in line)
            assert stock_count > 0 or 'BUY candidates' in chunk, \
    ...
    def test_header_included_in_all_chunks(self, mock_send):
        ...
        if mock_send.call_count > 1:
            for call_args in mock_send.call_args_list:
                chunk = call_args[0][0]
                assert 'BUY candidates' in chunk, \
                    "Each chunk should include the header for context"
            ...
            if stock_header in all_chunks_text:
                ...
                for call_args in mock_send.call_args_list:
                    ...
                    if stock_header in chunk:
                        ...
                        assert f"\tBuy (100-105)" in chunk or "Buy" in chunk.split(stock_header)[1].split(f"{i+1}.")[0]
                        break
