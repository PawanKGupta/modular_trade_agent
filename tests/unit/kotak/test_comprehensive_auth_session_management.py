...
from modules.kotak_neo_auto_trader.auth_handler import (  # noqa: E402
...
from modules.kotak_neo_auto_trader.live_price_cache import LivePriceCache  # noqa: E402
...
class TestReAuthentication(unittest.TestCase):
    ...
    def test_auth_error_detection(self):
        ...
        for response in test_cases:
            self.assertTrue(is_auth_error(response), f"Failed to detect auth error in: {response}")
    ...
    def test_non_auth_error_not_detected(self):
        ...
        for response in test_cases:
            self.assertFalse(
                is_auth_error(response), f"Should not detect auth error in: {response}"
            )
...
class TestAutoTradeEngineAuthHandling(unittest.TestCase):
    ...
    def test_place_new_entries_with_expired_auth(self):
        ...
        recs = [Recommendation(ticker="TESTSTOCK.NS", verdict="buy", last_close=100.0)]
...
class TestLivePriceCacheReconnection(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.price_cache = LivePriceCache(
            auth_client=self.mock_client, scrip_master=self.mock_scrip_master, auth=self.mock_auth
        )
    ...
    def test_reconnect_after_reauth(self):
        ...
        # Accept exchange parameter (added for user config support)
        def mock_get_instrument(symbol, exchange="NSE"):
            return {"token": 12345, "symbol": "TESTSTOCK-EQ", "exchange": exchange}
