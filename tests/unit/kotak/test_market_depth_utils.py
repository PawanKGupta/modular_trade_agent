...
def test_extract_market_depth_from_list_payload():
    payload = [
        {"fault": {"code": "400"}},
        {
            "depth": {
    ...
    assert snapshot.bid_levels[0] is not None
    assert snapshot.ask_levels[0] is not None
    assert snapshot.bid_levels[0].price == 99.5
    assert snapshot.ask_levels[0].price == 101.5


def test_format_premarket_depth_log_bids_and_asks():
    bid_levels = (
    ...
    assert "L1: Rs 1258.80" in msg
    assert "L2: Rs 1260.00" in msg
...
def test_format_premarket_ask_depth_log_no_liquidity():
    ...
    assert "asks (5 levels, no liquidity)" in msg
