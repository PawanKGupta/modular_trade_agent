...
class TestImmediateOrderVerification:
    ...
    def test_verify_order_placement_rejected(self, auto_trade_engine):
        ...
        auto_trade_engine.orders_repo.list.return_value = ([mock_db_order], 1)
        ...
        with patch("time.sleep"):
            is_valid, reason = auto_trade_engine._verify_order_placement(
                order_id="ORDER123",
                symbol="RELIANCE",
                wait_seconds=0,  # Avoid long sleep via patch
            )
    ...
    def test_verify_order_placement_database_update_failure(self, auto_trade_engine):
        ...
        # Mock telegram
        with (
            patch("modules.kotak_neo_auto_trader.auto_trade_engine.send_telegram"),
            patch("time.sleep"),
        ):
            is_valid, reason = auto_trade_engine._verify_order_placement(
                order_id="ORDER123",
