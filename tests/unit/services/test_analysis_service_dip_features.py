...
class TestAnalysisServiceIntegration:
    ...
    def test_analyze_ticker_includes_dip_features(
        ...
        mock_verdict.assess_fundamentals.return_value = (True, {'reason': 'Good fundamentals'})
        ...
        mock_verdict.determine_verdict.return_value = {
            'verdict': 'strong_buy',
            'justification': ['Strong reversal setup'],
            'volume_data': {
                'avg_vol': 1000000,
                'today_vol': 1500000,
                'vol_ok': True,
            ...
            'candle_analysis': 'Bullish reversal',
            'trading_params': {
                'buy_range': (950, 960),
                'target': 1050,
                'stop': 900
                ...
                with patch('services.analysis_service.IndicatorService') as mock_indicator_class:
                    with patch('services.analysis_service.SignalService') as mock_signal_class:
                        with patch('services.analysis_service.VerdictService') as mock_verdict_class:
                            ...
                            mock_verdict.assess_fundamentals.return_value = (True, {'reason': 'Good'})
