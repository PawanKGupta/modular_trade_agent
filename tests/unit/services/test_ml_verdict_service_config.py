...
from services.ml_verdict_service import MLVerdictService  # noqa: E402
...
class TestMLVerdictServiceConfigBasedModelSelection:
    ...
    def custom_config(self):
        ...
        return StrategyConfig(
            rsi_period=14, volume_exhaustion_lookback_daily=30, support_resistance_lookback_daily=50
        )
        ...
        with patch("utils.model_versioning.get_model_path") as mock_get_path:
            ...
            with patch("services.ml_verdict_service.Path") as mock_path_class:
                ...
                def path_side_effect(path_str):
                    mock_path = MagicMock()
                    if "random_forest" in str(path_str) or str(path_str).endswith(
                        "verdict_model_random_forest.pkl"
                    ):
    ...
    def test_init_with_custom_config(self, custom_config, temp_dir):
        ...
        with patch("utils.model_versioning.get_model_path") as mock_get_path:
            ...
            with patch("joblib.load") as mock_load:
                ...
                MLVerdictService(model_path=None, config=custom_config)
    ...
    def test_init_loads_feature_columns(self, default_config, temp_dir):
        ...
        with patch("utils.model_versioning.get_model_path") as mock_get_path:
            ...
            with patch("joblib.load") as mock_load:
                mock_model = MagicMock()
                # Align with legacy sidecar validation (n_features_in_ vs column count).
