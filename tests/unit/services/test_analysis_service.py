...
class TestAnalysisService:
    ...
    def test_analyze_ticker_flow(
        self, mock_verdict_class, mock_signal_class, mock_indicator_class, mock_data_class
    ):
        ...
        mock_verdict.determine_verdict.return_value = (
            "strong_buy",
            ["rsi:25(above_ema200)", "pattern:hammer"],
        )
        ...
        result = service.analyze_ticker(
            ticker="RELIANCE.NS", enable_multi_timeframe=False, export_to_csv=False
        )
    ...
    def test_initialization_uses_ml_verdict_service_when_model_exists(
        self, mock_ml_service_class, mock_path_class
    ):
        """Test that AnalysisService automatically uses MLVerdictService when ML model exists and ml_enabled=True"""
    ...
    def test_initialization_uses_verdict_service_when_model_not_exists(self, mock_path_class):
        ...
        assert (
            not hasattr(service.verdict_service, "model_loaded")
            or not service.verdict_service.model_loaded
        )

    @patch("pathlib.Path")
    @patch("services.ml_verdict_service.MLVerdictService")
    def test_initialization_falls_back_to_verdict_service_on_ml_init_error(
        self, mock_ml_service_class, mock_path_class
    ):
    ...
    @patch("pathlib.Path")
    @patch("services.ml_verdict_service.MLVerdictService")
    def test_initialization_falls_back_when_model_fails_to_load(
        self, mock_ml_service_class, mock_path_class
    ):
