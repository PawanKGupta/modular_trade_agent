...
def test_pnl_daily_and_summary(client, db_session):
    ...
    tokens = signup_and_verify_payload(
        client, db_session, {"email": unique_email, "password": "Secret123!"}
    )
    headers = {"Authorization": f"Bearer {tokens['access_token']}"}

    user = db_session.query(Users).filter(Users.email == unique_email).one()
    today = date.today()
    db_session.add(
        PnlDaily(
            user_id=user.id,
...
def test_targets(client, db_session):
