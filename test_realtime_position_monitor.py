...
open_positions = [
    trade for trade in history.get('trades', [])
...
for trade in open_positions:
    ...
    print(f"  - {symbol}: {qty} @ ₹{entry_price}")
print()
...
try:
    ...
    if monitor.price_manager:
        ...
        print("⚠️ Live price manager not available")
        print()
    ...
    if results.get('positions'):
        ...
        for pos in results['positions']:
            ...
            print(f"  P&L: ₹{pos.unrealized_pnl:,.0f} ({pos.unrealized_pnl_pct:+.2f}%)")
            print(f"  RSI10: {pos.rsi10:.1f}")
            print(f"  EMA9: ₹{pos.ema9:.2f}")
    ...
    if monitor.price_manager and monitor.price_manager.is_websocket_connected():
        ...
        print("  ⚠️ WebSocket not connected (will use yfinance fallback)")
    print()
