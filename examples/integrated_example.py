...
def print_comparison_summary(comparison_results):
    ...
    for stock, results in comparison_results.items():
        if results and results.get('total_signals', 0) > 0:
            ...
            print(f"{stock:<15} {signals:<8} {trades:<8} {return_pct:>7.1f}%   {vs_bh:>8.1f}%   {win_rate:>6.1f}%")
        else:
...
def print_period_comparison(period_results):
    ...
    print(f"{'Period':<20} {'Signals':<8} {'Trades':<8} {'Return%':<10} {'Win%':<8} {'vs B&H%':<10}")
    print("-" * 75)
    ...
    for period, results in period_results.items():
        if results and results.get('total_signals', 0) > 0:
            ...
            print(f"{period:<20} {signals:<8} {trades:<8} {return_pct:>7.1f}%   {win_rate:>6.1f}%   {vs_bh:>8.1f}%")
        else:
...
def main():
    ...
    print("This demonstrates the coordinated workflow between backtesting and trade agent analysis.")
    print("=" * 80)
